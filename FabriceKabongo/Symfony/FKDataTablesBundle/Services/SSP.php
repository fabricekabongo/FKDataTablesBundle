...
namespace FabriceKabongo\Symfony\FKDataTablesBundle\Services;
 ...
 * 
 * Modified by Fabrice Kabongo (it's now instanciable)
 *
...
class SSP {
     ...
     *  @param  array $request Data sent to server by DataTables
     *  @param  array $columns Column information array
     *  @param  array $bindings Array of values for DBAL bindings, used in the
     *    sql_exec() function
     ...
     *
     *  @param  array $request Data sent to server by DataTables
     *  @param  array|DBAL $conn DBAL connection resource or connection parameters array
    ...
    public function simple($request, $table, $primaryKey, $columns) {
        ...
        $resFilterLength = $this->sql_exec("SELECT FOUND_ROWS()"
        );
        $recordsFiltered = $resFilterLength[0][0];

        // Total data set length
        $resTotalLength = $this->sql_exec("SELECT COUNT( {$primaryKey} )
			 FROM    $table "
        );
    ...
    public function complex($request, $table, $primaryKey, $columns, $whereResult = null, $whereAll = null) {
        ...
        // Total data set length
        $resTotalLength = $this->sql_exec($bindings, "SELECT COUNT( {$primaryKey} )
			 FROM    $table  " .
                $whereAllSql
        );
        $recordsTotal = $resTotalLength[0][0];
     ...
     *
     * @param  array    $bindings Array of DBAL binding values from bind() to be
    ...
    public function sql_exec($bindings, $sql = null) {
        ...
        if (is_array($bindings)) {
            for ($i = 0, $ien = count($bindings); $i < $ien; $i++) {
                $binding = $bindings[$i];
                $l[$binding['key']] =  $binding['val'];
            }
        }
        // Execute
        try {
            $stmt = $this->conn->executeQuery($sql,$l);
        } catch (DBALException $e) {
            $this->fatal("An SQL error occurred: " . $e->getMessage());
        }

        // Return all
        $k = $stmt->fetchAll(\PDO::FETCH_BOTH);
        return $k;
    }
    ...
    /**
     * Create a DBAL binding key which can be used for escaping variables safely
     * when executing a query with sql_exec()
     *
     * @param  array &$a    Array of bindings
     * @param  *      $val  Value to bind
     * @param  int    $type DBAL field type
